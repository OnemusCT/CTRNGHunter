...
bool RNGHunter::parseFile(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (funcName == "load") {
            ...
            for (size_t i = 0; i < rng_sim_pool_.size(); i++) {
                functions_[i].emplace_back(
                    [this, i, thresholds, min_crits, max_turns, battle_name](RNGSim::LogLevel log_level) {
                        return rng_sim_pool_[i]->battle_with_crits(thresholds, min_crits, max_turns, battle_name,
                                                                   log_level);
                    });
            }
...
void RNGHunter::generateWalkthrough(time_t seed, std::ostream& out) {
    ...
    generate_walkthrough(seed, rng_sim_pool_[0]->get_battle_rng_per_encounter(),
                         rng_sim_pool_[0]->get_extra_rooms_per_encounter(),
                         rng_sim_pool_[0]->get_extra_heals_per_encounter(), out);
}
...
void RNGHunter::logSeedFromFunctions(time_t seed, const std::vector<RNGSimFunc>& functions,
...
std::vector<RNGSimFunc> RNGHunter::findSeedHelper(int sim_index, time_t seed, int allowable_heals,
...
std::unordered_map<time_t, std::vector<RNGSimFunc>> RNGHunter::findSeeds(
        ...
        threads.emplace_back(
            [this, i, thread_start, thread_end, allowable_heals, allowable_room_pairs, &thread_results, &statistics,
                ...
                for (time_t seed = thread_start; seed <= thread_end; ++seed) {
                    ...
                    std::vector<RNGSimFunc> curr_results = findSeedHelper(
                        i, seed, allowable_heals, allowable_room_pairs, debug);
