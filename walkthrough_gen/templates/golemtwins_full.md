**Golem Twins (7000 HP)**
{{<battlebox>}}Crono Lightning II  
Ayla Boogie (repeat if miss)  
Crono / Ayla Falcon Strike x5  
Robo Mid Tonic self if < 280 HP{{</battlebox>}}

Continue, talk to Nu, continue  
After everyone is beaten, approach Lavos, F  
Mash Cancel to skip FMV

## Dalton Plus

### Surviving Village

**Menu**  
Party (Lucca / Ayla / Robo)

* Marle (3rd) <-> Lucca (Confirm, Right, Down, Confirm)
* Lucca -> Robo (Confirm, Up, Confirm)
* Lucca -> Ayla (Confirm, Up, Confirm)

Go out and enter top hut, talk to Nu

**Shop**  
Buy

* Shelter x11
* Athenian Water x21
* Hi-Potion x41 (or rest)

Go out and enter Village Commons, talk to Elder

### Blackbird

Go up the stairs then back

Vents

* Go right
* Go up on first fork
* All the way top then left

Go left and press switch  
Go down (hugging right wall) and enter the room

**Byte, Daltonite x2**
{{<battlebox>}}Ayla Cat Attack Byte (Left)
Auto-battle{{</battlebox>}}

**Get the inventory back** from the chest

**Menu**  
Equipment

* Lucca (**Airgun** / \- / \- / \-)
  * Airgun
* Robo (**Kaiser Arm** / \- / \- / \-)
  * Kaiser Arm

Inventory

* Sort list
* Potion <-> Shelter
* Hi-Potion everyone (if needed)

Settings

* Battle Speed: 1

Go thought the back door and then up the ladder on top  
**Default option**, hold down+left, continue left
