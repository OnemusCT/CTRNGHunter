## Golem Twins

### Terra Cave

Talk to Melchior after Dalton leaves to and **get Ruby Knife**  
Go up the stairs then right and leave  
Go all the way right and take portal until Zeal Palace (same way as before)

### Zeal Palace

Go left and go on the heal tile  
Talk to plant lady and select the **1st option** ("Yes") then the **2nd option** ("Grow it in secret")

* After 1st option, keep moving that the 2nd one will be auto triggered

Go up and left then enter the middle hall

**Dalton (3500 HP)**

* Crono Falcon Strike  
* Ayla Falcon Strike  
* Lucca Fire  
* Falcon Strike

### Ocean Palace

**Menu**  
Equipment

* Crono (Swallow / Stone Helm / **Mesozoic Armor** / Power Scarf)  
  * Mesozoic Mail

Inventory

* Shelter

Talk to Mune then go to the next screen  
Go right, enter door, step on the switch

**Djinn, Ghul**

* Escape

Take bottom-right exit to the next screen, left to main room  
**Pick Aeonian Suit** and go down to the next screen

Go down, **pick Kaiser Arm**, take left path and step on the switch

* Walk back a little bit to align party

Go back and go back to the main room (careful with Blue Scouters)

Go left and down to the next screen  
Go left and down the stairs

**Zealot Mage x2, Monk x2**

* Escape

Run while enemies jumping (1-1-2)  
Escape battle if hit any  
Continue down the stairs

**Warrior x2, Monk x2**

* Falcon Strike

Continue down

**Warrior x2, Monk x2**

* Falcon Strike  
  * If hit 4: Done  
  * If hit 3: Move cursor to Ayla then Auto Battle  
  * If hit 2: Falcon Strike  
* Should learn Lightning II

Talk to Masa then continue to the elevator

**Zealot Mage, Blue Scouter x2, Scouter x2**

* Falcon Strike top row (starts on Cyclone)  
* Falcon Strike bottom row

Go back inside and repeat fight (Lucca should learn Fire II)

**Zealot Mage, Blue Scouter x2, Scouter x2**

* Falcon Strike top row  
* Falcon Strike bottom row

**Menu**  
Party (Crono / Robo / Ayla)

* Lucca <-> Robo (Confirm, Right, Down, Confirm)

Examine the orb

**Djinn, Ghul, Monk x3**

* Falcon Strike bottom row (Left)  
* Falcon Strike top row  
  * Robo Mid-Potion if needed  
* Move cursor to Ayla then Auto Battle  
  * Can do Falcon Strike if only 1 on last stairs fight

Ayla should learn Charm

**Zealot Mage x2, Warrior x2, Scouter**

* Falcon Strike top row  
* Falcon Strike bottom row

Robo Tackle, Boogie and Spin Kick learned

**Red Scouter x2, Blue Scouter x2, Scouter x2**

* Falcon Strike bottom row  
* Falcon Strike top row

Go inside, go left and hit the switch (escape from scouters, **don't attack**)  
Go back, go right and hit the switch (escape from scouters, **don't attack**)  
Go back, step on middle switch and continue

**Menu**  
Inventory

* Shelter

**Pick Elixir**

**Golem Twins (7000 HP)**

* Lightning II  
* Boogie (repeat if miss)  
* Falcon Strike x5  
* Robo Mid Tonic self if \< 280 HP

Continue, talk to Nu, continue  
After everyone is beaten, approach Lavos, F  
Mash Cancel to skip FMV
